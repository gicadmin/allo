CREATE OR REPLACE FORCE VIEW phenix.v_pfa_base_sdb (prd_code,cur_prf_fmt_code,prd_definition,lty_whs_code,real_inv_quantity_on_hand,real_inv_quantity_on_hold,lty_transit_switch,lty_receiving_dock_switch,lty_shipping_dock_switch,lty_damage_switch,lty_cash_and_carry_switch,lty_picking_switch,lty_overstock_switch,lty_liquidation_switch,prf_average_weight) AS
SELECT prd.prd_code prd_code,
          current_prf.prf_fmt_code cur_prf_fmt_code,
          prd_definition,
          lty_whs_code,
          NVL (inv_quantity_on_hand, 0) real_inv_quantity_on_hand,
          NVL (inv_quantity_on_hold, 0) real_inv_quantity_on_hold,
          lty_transit_switch,
     ...
     FROM products prd
          ...
          JOIN
          v_inventories inv
             ON (    inv.inv_prf_prd_code = prd.prd_code
                 AND inv.inv_prf_fmt_code = fmt.fmt_code)
